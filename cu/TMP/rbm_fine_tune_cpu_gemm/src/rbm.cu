...
int DeepBeliefNet::gradient_gpu(GradientContext& ctx, const Vector& weights, Vector& weight_incs, float& cost)
{
	...
	// delete below
	printf("Cannot execute gradient_gpu()\n");
	exit(0);
	...
	/* Fill arguments
	cudaMalloc((void **)&d_buf_g, );
	cudaMalloc((void **)&d_weight_g, );
	cudaMalloc((void **)&d_hidden_bias_g, );
	cudaMalloc((void **)&d_diff_g, );
	cudaMalloc((void **)&d_weight_incs_g, );
	*/

	// Initialize weight incs to zero

	/* Fill arguments
	cudaMemset(d_weight_incs_g, );
	*/
	...
	for (int i = 1; i <= max_layer; ++i) {
		...
		if (i == 1 || weights.empty()) {
			// Copy weight & bias data from host to device

			/* Fill arguments
			cudaMemcpy(d_weight_g + ofs_w[i-1], );
			cudaMemcpy(d_hidden_bias_g + ofs_h[i-1], );
			*/
			...
			// Copy weight data from host to device

			/* Fill arguments (weight copy)
			cudaMemcpy(d_weight_g + ofs_w[i-1], );
			*/
			start = end;
			end = start + rbm->num_hidden();

			/* Fill arguments (bias copy)
			cudaMemcpy(d_hidden_bias_g + ofs_h[i-1], );
			*/
		}
	...
	*/ 
	...
	for (int layer=0; layer < max_layer; ++layer) {
		...
		*/ 
		...
		// Launch kernel. It is the same as GEMM_NT
