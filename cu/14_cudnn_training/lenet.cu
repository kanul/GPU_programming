...
DEFINE_int32(random_seed, 2018, "Override random seed (default uses std::random_device)");
...
// Solver parameters
DEFINE_double(learning_rate, 0.05, "Base learning rate");
...
struct TrainingContext
{
    ...
    {    
        ...
        // alpha           (float):                           1.0f
        // beta            (float):                           0.0f
        // conv2Tensor     (cudnnTensorDescriptor_t):         Tensor Descriptor for Forward propagation result of Convolution 2 layer.
        // conv2BiasTensor (cudnnTensorDescriptor_t):         Tensor Descriptor for Bias of Convolution 2 layer.
        // pool1Tensor     (cudnnTensorDescriptor_t):         Tensor descriptor for Forward propagation result of Pool 1 layer.
        // pconv2          (float*):                          GPU memory pointer to Weight of Convolution 2 layer.
        // gconv2bias      (float*):                          GPU memory pointer to Gradient for Filter's Bias of Convolution 2 layer.
        // gconv2          (float*):                          GPU memory pointer to Gradient for Filter of Convolution 2 layer.
        // dconv2          (float*):                          GPU memory pointer to Gradient for Forward output of Convolution 2 layer.
        // pool1           (float*):                          GPU memory pointer to Forward propagation result of Pool 1 layer.
        // dpool2          (float*):                          GPU memory pointer to Gradient for Forward output of Pool 2 layer.
        // conv2Desc       (cudnnConvolutionDescriptor_t):    Convolution Descriptor of Convolution 2 layer.
        // conv2filterDesc (cudnnFilterDescriptor_t):         Filter Descriptor of Convolution 2 layer.
        // conv2bwfalgo    (cudnnConvolutionBwdFilterAlgo_t): Backward Filter Algorithm.
        // conv2bwdalgo    (cudnnConvolutionBwdDataAlgo_t):   Backward Data Algorithm.
        // workspace       (void*):                           GPU memory pointer to Workspace.
        // m_workspaceSize (size_t):                          Size of workspace.

				// Compute derivative with respect to bias. (output: gconv2bias)
        checkCUDNN(cudnnConvolutionBackwardBias  (cudnnHandle, /* TODO */ ));



				// Compute derivative with respect to weights. (output: gconv2)
        checkCUDNN(cudnnConvolutionBackwardFilter(cudnnHandle, /* TODO */ ));



				// Compute derivative with respect to data(for previous layer). (output: dconv2)
        checkCUDNN(cudnnConvolutionBackwardData  (cudnnHandle, /* TODO */ ));
