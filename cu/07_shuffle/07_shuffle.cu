...
__global__ void getMax_local_suffle(unsigned int *d_data, unsigned int *d_max, int n);
...
int main(int argc, char* argv[]){
	...
	gettimeofday(&start, NULL);
	getMax_local_suffle<<< grid, threads >>>(d_data, d_max, N);
	cudaDeviceSynchronize();
	cudaMemcpy(&max, d_max, sizeof(unsigned int), cudaMemcpyDeviceToHost);
	...
	printf("Local_suffle GPU, elapsed time: %lf\n", (timer.tv_usec / 1000.0 + timer.tv_sec * 1000.0));
	printf("Local_shuffle GPU, max value : %d\n", max);
...
__global__ void getMax_local_suffle(unsigned int *d_data, unsigned int *d_max, int n){
	...
	int value = d_data[gid];
	int value2;
	for(int i = 16; i > 0; i>>=1){
		value2 = __shfl_down(value, i, 32);
		if(value < value2)
			value = value2;
	}
	__syncthreads();
