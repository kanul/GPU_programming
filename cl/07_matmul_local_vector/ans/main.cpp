...
int main(int argc, char* argv[])
{
	...
	cl_kernel           kernel_shared_vector;
	...
	err = clGetDeviceIDs(firstPlatformId[0], CL_DEVICE_TYPE_GPU, 1,
		&device_id, NULL);
	CHECK_ERROR(err);

	cl_context_properties context_props[] =
	{
		CL_CONTEXT_PLATFORM, (cl_context_properties)firstPlatformId[0], 0
	};
	...
	strcpy_s(kernel_name, "matmulGPU_global");
	kernel_global = clCreateKernel(program, kernel_name, &err);
	CHECK_ERROR(err);
	strcpy_s(kernel_name, "matmulGPU_shared");
	kernel_shared = clCreateKernel(program, kernel_name, &err);
	CHECK_ERROR(err);
	strcpy_s(kernel_name, "matmulGPU_shared_vector");
	kernel_shared_vector = clCreateKernel(program, kernel_name, &err);
	CHECK_ERROR(err);
	...
	*************************************************/
	...
	size_t global[] = { (size_t)roundUp(local[0], N), (size_t)roundUp(local[1], N) };
	// number of total work-items

	// Set the arguments to our compute kernel
	err = 0;
	err |= clSetKernelArg(kernel_global, 0, sizeof(cl_mem), &d_a);
	err |= clSetKernelArg(kernel_global, 1, sizeof(cl_mem), &d_b);
	err |= clSetKernelArg(kernel_global, 2, sizeof(cl_mem), &d_r);
	err |= clSetKernelArg(kernel_global, 3, sizeof(int), &N);
	CHECK_ERROR(err);
	...
	printf("GPU (global memory) elapsed time: %.3lf\n", etimer);
	...
	*************************************************/
	...
	/*************************************************
	GPU Matrix multiplication using local memory
	*************************************************/
	local[0] = local[1] = TILE_WIDTH;
	global[0] = global[1] = (size_t)roundUp(TILE_WIDTH, N);

	if (N % TILE_WIDTH != 0) {
		puts("N must be the multiple of TILE_WIDTH when using shared memory");
		goto out;
	}

	// Set the arguments to our compute kernel
	err = 0;
	err |= clSetKernelArg(kernel_shared, 0, sizeof(cl_mem), &d_a);
	err |= clSetKernelArg(kernel_shared, 1, sizeof(cl_mem), &d_b);
	err |= clSetKernelArg(kernel_shared, 2, sizeof(cl_mem), &d_r);
	err |= clSetKernelArg(kernel_shared, 3, sizeof(int), &N);
	err |= clSetKernelArg(kernel_shared, 4, sizeof(float) * TILE_WIDTH * TILE_WIDTH, NULL);
	err |= clSetKernelArg(kernel_shared, 5, sizeof(float) * TILE_WIDTH * TILE_WIDTH, NULL);
	CHECK_ERROR(err);
	...
	printf("GPU (shared memory) elapsed time: %.3lf\n", etimer);
	...
	*************************************************/
	...
	/*************************************************
	GPU Matrix multiplication using local memory and vector
	*************************************************/
	local[0] = local[1] = TILE_WIDTH;
	global[0] = global[1] = (size_t)roundUp(TILE_WIDTH, N);
	...
	// Set the arguments to our compute kernel
	err = 0;
	err |= clSetKernelArg(kernel_shared_vector, 0, sizeof(cl_mem), &d_a);
	err |= clSetKernelArg(kernel_shared_vector, 1, sizeof(cl_mem), &d_b);
	err |= clSetKernelArg(kernel_shared_vector, 2, sizeof(cl_mem), &d_r);
	err |= clSetKernelArg(kernel_shared_vector, 3, sizeof(int), &N);
	err |= clSetKernelArg(kernel_shared_vector, 4, sizeof(float) * TILE_WIDTH * TILE_WIDTH, NULL);
	err |= clSetKernelArg(kernel_shared_vector, 5, sizeof(float) * TILE_WIDTH * TILE_WIDTH, NULL);
	CHECK_ERROR(err);

	err = clEnqueueNDRangeKernel(commands, kernel_shared_vector, ndim, NULL,
		global, local, 0, NULL, &event);
	CHECK_ERROR(err);
	...
	printf("GPU (shared memory/vector) elapsed time: %.3lf\n", etimer);
