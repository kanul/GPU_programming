...
__kernel void matmulGPU_shared_vector(__global float *a, __global float *b, __global float *r,int n, 
								__local float* s_a, __local float* s_b)
{
	int bx = get_group_id(0);
	int by = get_group_id(1);
	...
	for(int i=0; i < n/TILE_WIDTH; i++){
		s_a[ty * TILE_WIDTH + tx] = a[y * n + (i * TILE_WIDTH  + tx)];
		s_b[tx * TILE_WIDTH + ty] = b[(i * TILE_WIDTH  + ty) * n + x];
		barrier(CLK_LOCAL_MEM_FENCE);

		for(int j =0; j < TILE_WIDTH/4; j++){
			float4 temp_a = vload4(j, s_a + ty * TILE_WIDTH);
			float4 temp_b = vload4(j, s_b + tx * TILE_WIDTH);
			sum += dot(temp_a, temp_b);
		}
		barrier(CLK_LOCAL_MEM_FENCE);
	}
	r[ y * n + x] = sum;
