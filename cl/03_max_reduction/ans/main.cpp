...
int main(int argc, char* argv[])
{
	...
	srand(time(NULL));
	for (int i = 0; i<N; i++) {
		data[i] = (unsigned int)(rand() * rand() % 10000000);
	}
	...
	d_data = clCreateBuffer(context, CL_MEM_READ_ONLY,
		sz, NULL, NULL);
	d_max = clCreateBuffer(context, CL_MEM_READ_ONLY,
	...
	strcpy_s(kernel_name, "getMax_global");
	kernel_global = clCreateKernel(program, kernel_name, &err);
	CHECK_ERROR(err);
	strcpy_s(kernel_name, "getMax_local");
	kernel_local = clCreateKernel(program, kernel_name, &err);
	CHECK_ERROR(err);
	strcpy_s(kernel_name, "getMax_binary");
	...
	*************************************************/
	...
	max = 0;
	err = clEnqueueWriteBuffer(commands, d_max, CL_TRUE, 0,
		sizeof(unsigned int), &max, 0, NULL, NULL);
	CHECK_ERROR(err);

	// Set the arguments to our compute kernel
	err = 0;
	err |= clSetKernelArg(kernel_global, 0, sizeof(cl_mem), &d_data);
	err |= clSetKernelArg(kernel_global, 1, sizeof(cl_mem), &d_max);
	...
	err = clEnqueueReadBuffer(commands, d_max, CL_TRUE, 0,
		sizeof(unsigned int), &max, 0, NULL, NULL);
	CHECK_ERROR(err);
	...
	*************************************************/
	...
	// Set the arguments to our compute kernel
	err = 0;
	err |= clSetKernelArg(kernel_local, 0, sizeof(cl_mem), &d_data);
	err |= clSetKernelArg(kernel_local, 1, sizeof(cl_mem), &d_max);
	err |= clSetKernelArg(kernel_local, 2, sizeof(int), &N);
	err |= clSetKernelArg(kernel_local, 3, sizeof(unsigned int), NULL);

	//Time measure start
	err = clEnqueueNDRangeKernel(commands, kernel_local, ndim, NULL,
		global, local, 0, NULL, &event);
	CHECK_ERROR(err);
	...
	*************************************************/
	...
	err |= clSetKernelArg(kernel_binary, 1, sizeof(cl_mem), &d_max);
	...
	printf("Binary reduction, max value : %d\n", max);
out:
