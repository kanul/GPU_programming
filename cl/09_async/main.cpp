...
#define nstreams 4 // Set nstreams to 1, if not using ASYNC MODE
	...
	if (nstreams == 1) {
		...
		if (async_mode == ASYNC_V1) {
			...
			for (int i = 0; i < niterations; i += nstreams) {
				...
				for (int j = 0; j < nstreams; j++) {
					// TODO : Data Copy To Device -> Enqueue Kernel -> Data Copy To Host 
			...
			for (int i = 0; i < niterations; i += nstreams) {
				// H2D memcpy
				for (int j = 0; j < nstreams; j++) {
					// TODO : Data Copy To Device for each command queue
				}
				// Kernel call
				for (int j = 0; j < nstreams; j++) {
					// TODO : Enqueue Kernel for each command queue
				}
				// copy the result D2H(device to host)
				for (int j = 0; j < nstreams; j++) {
					// TODO : Data Copy To Host for each command queue
				}
			}
		}
		else
		{
			...
			for (int i = 0; i < niterations; i += nstreams) {
				...
				for (int j = 0; j < nstreams; j++) {
					// TODO : Enqueue Kernel -> Data Copy To Host for each command queue
				}
			}
		}
	}

	// TODO : Waits for all works in commands queue end
